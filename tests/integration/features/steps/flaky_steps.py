"""Steps for flaky integration tests."""
...
def step_flaky_fail_twice(context):
    attempts = _get_attempt("flaky_fail_twice")
    if attempts < 3:
        raise AssertionError(f"Flaky failure on attempt {attempts}")


@given("a flaky scenario with name {name}")
def step_flaky_named(context, name):
    attempts = _get_attempt(f"flaky_{name}")
    if attempts < 2:
        raise AssertionError(f"Flaky failure for {name} on attempt {attempts}")
